...
BlocklyStorage.link = function() {
  ...
  BlocklyStorage.makeRequest_('https://gogoapi.learninginventions.org/storage/v2/', 'xml', data, 'link');
};
...
BlocklyStorage.retrieveXml = function(key) {
  var xml = Blockly.Xml.workspaceToDom(Blockly.getMainWorkspace());
  BlocklyStorage.makeRequest_('https://gogoapi.learninginventions.org/storage/v2/', 'key', key, 'get');
};
    ...
    if (BlocklyStorage.httpRequest_.status != 200) {
      ...
      if (BlocklyStorage.httpRequest_.name == 'xml') {
        ...
        if (BlocklyStorage.httpRequest_.action=='link'){
          ...
          BlocklyStorage.alert(BlocklyStorage.LINK_ALERT.replace('%1',
          host + '#/app/program?id='+data));
          IframeMessenger.sendMessage({event:'updateFileId', data: data});
        }
