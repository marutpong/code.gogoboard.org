...
var blockSpatial = new Class({
  initialize: function() {
    ...
    that = new Element('div', {
      class: 'spatialBlock',
      html:
        '<div data-type="title" class="title"></div><div data-type="var1" class="var1"></div><div data-type="var2" class="var2"></div>'
    })
    ...
    that.addEvent('click', function() {
      if (!that.hasClass('selected')) {
        that
          .getParent('.contentSpatial')
          .getElements('.spatialBlock')
    ...
    that.getElements('div').addEvent('dblclick', function(event) {
      ...
      if (!item.hasClass('selected')) {
        ...
        item.getFirst().addEvent('blur', function() {
          item
            .getParent('.contentSpatial')
            .fireEvent('changeBlockContent', item)
...
var initSpatial = function() {
  ...
  mapping.addBlockToToolbox = function(block) {
    ...
    var newBlock = new Element('block', {
      id: blockID,
      html:
        '<mutation name="' +
        block.title +
  ...
  mapping.getElement('.deleteBlock').addEvent('click', function() {
    if (mapping.data.length > 1) {
      ...
      mapping.currentData =
        mapping.currentData == 0 ? 0 : mapping.currentData - 1
  ...
  condition
    .getElement('.mainArea h3')
    .set(
      'html',
      'Amount inputs: &nbsp;&nbsp;&nbsp;<input id="radioOneInput" type="radio" checked name="amountInput" /> 1, &nbsp;&nbsp;&nbsp;&nbsp;<input id="radioTwoInput" type="radio" name="amountInput" /> 2'
    )
  ...
  condition.addBlockToToolbox = function(block) {
    ...
    if (condition.json[blockIndex]['ssX'].type == 'Variable') {
      xStr1 =
        '<arg name="' + condition.json[blockIndex]['ssX'].name + '"></arg>'
      xStr2 =
        '<value name="ARG' +
        varCount +
        '"><block type="variables_get"><field name="VAR">' +
        condition.json[blockIndex]['ssX'].name +
        '</field></block></value>'
      varCount++
    }
    ...
    if (condition.json[blockIndex]['ssY'].type == 'Variable') {
      yStr1 =
        '<arg name="' + condition.json[blockIndex]['ssY'].name + '"></arg>'
      yStr2 =
        '<value name="ARG' +
        varCount +
        '"><block type="variables_get"><field name="VAR">' +
        condition.json[blockIndex]['ssY'].name +
        '</field></block></value>'
    }
    ...
    block.area.each(function(item) {
      ...
      if (areaArr[4] && areaArr[4].clean() != '') {
        ...
        var newBlock = new Element('block', {
          id: blockID,
          html: '<field name="VAR">' + areaName + '</field>'
        })
  ...
  condition.updateBlock = function() {
    ...
    xmlText = xmlText
      ...
      .join(block.title)
      .split('[VAR1]')
      .join(block.type + block.var1)
      .split('[VAR2]')
      .join(block.type + block.var2)
  ...
  condition.getElement('.deleteBlock').addEvent('click', function() {
    if (condition.json.length > 1) {
      ...
      condition.currentData =
        condition.currentData == 0 ? 0 : condition.currentData - 1
  ...
  condition.setType = function(type) {
    condition.type = type
    condition.spatial.setType(
      type,
      condition.currentBlock.var1,
      condition.currentBlock.var2
    )
  }
  ...
  $$('#radioPopupSensor, #radioPopupVariable, #radioPopupSwitch').addEvent(
    'change',
    function() {
  ...
  document.addEvent('showInputPopup', function(axis) {
    ...
    $(
      'radioPopup' + condition.json[condition.currentData]['ss' + axis].type
    ).checked = true
    $(
      'radioPopup' + condition.json[condition.currentData]['ss' + axis].type
    ).fireEvent('change')
    if (condition.json[condition.currentData]['ss' + axis].type == 'Variable') {
      $('inputVariableName').set(
        'value',
        condition.json[condition.currentData]['ss' + axis].name
      )
      $('inputVariableMin').set(
        'value',
        condition.json[condition.currentData]['ss' + axis].min
      )
      $('inputVariableMax').set(
        'value',
        condition.json[condition.currentData]['ss' + axis].max
      )
    } else {
      $('inputPopupLeft')
        .getChildren()
...
document.addEvent('spatialIsReady', function() {
  ...
  var list = new Element('div', {
    class: 'conditionType',
    html:
      '<div data-type="onesensor">1 sensor</div><div data-type="sensor">2 sensors</div><div data-type="switch">2 switches</div>'
  })
...
var normalWS = function() {
    ...
    if (resp[0] == 'burst') {
      ...
      if ($('sensorOptionBTN').hasClass('selected')) {
        ...
        for (var i = 0; i < 8; i++) {
          ...
          $('sensor' + (i + 1))
            .getFirst('.gate')
            .setStyle('height', 100 - rs[i] / 10.24 + '%')
          $('sensor' + (i + 1))
            .getElements('.sensorValue')
            .set('text', rs[i])
          //}
        }
      }
  ...
  ws.onclose = function(status) {
    ...
    new Element('div', {
      id: 'warningMessage',
      class: 'anime12',
      text: 'Cannot connect to the GoGo Widget. Retrying..'
