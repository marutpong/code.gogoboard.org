...
Code.loadXML = function (event) {
  ...
  reader.onloadend = function (event) {
    ...
    if (target.readyState == 2) {
      ...
      if (isReplace) {
        IframeMessenger.sendMessage({ event: 'responseOpen', data: target.result })
      } else {
        IframeMessenger.sendMessage({ event: 'responseImport', data: target.result })
      }
    }
...
Code.saveXML = function () {
  ...
  if (IframeMessenger && IframeMessenger.isProduction) {
    console.log(data)
  }
...
Code.discard = function () {
  ...
  if (count < 2 ||
    ...
    IframeMessenger.sendMessage({ event: 'responseNewfile', data: true })
  }
}
...
Code.saveRevision = function () {
  ...
  var data = Blockly.Xml.domToText(xml)
  BlocklyStorage.makeRequest_('https://gogoapi.learninginventions.org/storage/v2/', 'xml', data, 'download')
}
